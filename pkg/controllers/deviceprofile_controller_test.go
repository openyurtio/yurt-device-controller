...
var _ = Describe("Device controller", func() {
	...
	Context("When updating DeviceProfile Status", func() {
		It("Should trigger DeviceProfile instance", func() {
			...
			rop := devicev1alpha1.ResourceOperation{
				DeviceResource: "EnableRandomization_Bool",
				DefaultValue:   "false",
			}

			cmd := devicev1alpha1.DeviceCommand{
				Name:               "WriteBoolValue",
				IsHidden:           false,
				ReadWrite:          "W",
				ResourceOperations: []devicev1alpha1.ResourceOperation{rop},
			}

			resp := devicev1alpha1.ResourceProperties{
				ReadWrite:    "W",
				DefaultValue: "true",
				ValueType:    "Bool",
			}

			res := devicev1alpha1.DeviceResource{
			...
			dp := &devicev1alpha1.DeviceProfile{
				...
				Spec: devicev1alpha1.DeviceProfileSpec{
					...
					Model:           Model,
					DeviceCommands:  []devicev1alpha1.DeviceCommand{cmd},
					DeviceResources: []devicev1alpha1.DeviceResource{res},
				},
			}
			Expect(k8sClient.Create(ctx, dp)).Should(Succeed())

			lookupKey := types.NamespacedName{Name: ProfileName, Namespace: CommonNamespace}
			created := &devicev1alpha1.DeviceProfile{}
